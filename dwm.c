...
// PATCH_vanitygaps static void tile(Monitor *m);
...
static int screen;
static int sw, sh; /* X dis/updateplay screen geometry width, height */
...
void cyclelayout(const Arg* arg)
{
    ...
    if (arg->i > 0)
    {
        if (l->symbol && (l + 1)->symbol) setlayout(&((Arg){.v = (l + 1)}));
        else setlayout(&((Arg){.v = layouts}));
    }
    else
    {
        if (l != layouts && (l - 1)->symbol) setlayout(&((Arg){.v = (l - 1)}));
        else setlayout(&((Arg){.v = &layouts[LENGTH(layouts) - 2]}));
    }
}
...
void manage(Window w, XWindowAttributes* wa)
{
    ...
    attachaside(c); // SEARCHTAG -attach(c);
    attachstack(c);
...
void unmanage(Client* c, int destroyed)
{
    ...
    for (i = 0; i < LENGTH(tags) + 1; i++)
        if (c->mon->pertag->sel[i] == c) c->mon->pertag->sel[i] = NULL;
...
void view(const Arg* arg)
{
    ...
    if (arg->ui & TAGMASK)
    {
        ...
        if (arg->ui == ~0) selmon->pertag->curtag = 0;
        else
        {
